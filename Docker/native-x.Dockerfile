...
RUN ./mvnw -s settings.xml -Pnative clean package -DskipTests

## UPX
FROM gruebel/upx:latest as upx
COPY --from=builder /app/target/beneficiaires /beneficiaires

RUN upx --best --lzma /beneficiaires
## UPX
...
COPY --from=upx /beneficiaires /beneficiaires

ENTRYPOINT ["/beneficiaires"]
